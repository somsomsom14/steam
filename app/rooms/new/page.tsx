...
import { RoomNewClient } from "./RoomNewClient";
...
export default async function RoomNewPage() {
  ...
  const [{ data: games }, { data: user }] = await Promise.all([
    supabase.from("user_games").select("appid, game_name").eq("user_id", session.userId).order("game_name", { ascending: true }),
